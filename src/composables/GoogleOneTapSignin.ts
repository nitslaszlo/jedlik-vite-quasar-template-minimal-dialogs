...
export default function () {
  const CLIENT_ID = import.meta.env.VITE_VUE_APP_CLIENT_ID;
  const CLIENT_URL = import.meta.env.VITE_VUE_APP_CLIENT_URL;
  ...
  const googleOneTap = (
    { client_id, auto_select = false, cancel_on_tap_outside = false, context = "signin" }: any,
    callback: any
  ) => {
  ...
  const oneTapSignin = (options: any) => {
    googleOneTap(options, (res: any) => {
